...
* **Correct opt-out filtering** – the original used ``('opt_out', '=', False)``
  ...
  version queries ``mailing.contact.subscription`` directly so only contacts
  that are actively subscribed (and not opted out) of the *target* lists are
  included.
...
class SmsAtTemplate(models.Model):
    ...
    def action_send_to_lists(self) -> dict:
        ...
        * Has an active subscription (``opt_out = False``) to at least one
          of the template's ``mailing_list_ids``.
        * Has a non-empty ``mobile`` field.
        ...
        # ----------------------------------------------------------------
        # Query subscriptions to get correctly opt-out-filtered contacts.
        # Using mailing.contact.subscription directly is more accurate than
        # relying on the computed opt_out field on mailing.contact, which
        # may aggregate across all lists the contact belongs to.
        # ----------------------------------------------------------------
        subscriptions = self.env["mailing.contact.subscription"].search(
            [
                ("list_id", "in", self.mailing_list_ids.ids),
                ("opt_out", "=", False),
            ]
        )
        ...
        if not subscriptions:
            raise UserError(
                _(
                    "No active subscribers found in the selected mailing list(s).  "
                    "Make sure the lists contain opt-in contacts."
                )
            )
